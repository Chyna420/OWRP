...
function PropPlacer(ObjectModel)
    ...
    while true do
        ...
        if PromptHasHoldModeCompleted(SetPrompt) then
            ...
            TriggerServerEvent("vorp_propplacer:SaveProp" ,PropHash ,  pPos.x , pPos.y , pPos.z , heading )
...
RegisterCommand("1899", function(source, args, rawCommand)
    PropPlacer(args[1])
end)
local StopPlace = false
RegisterCommand("1900", function(source, args, rawCommand)
    StopPlace = true
    for k,v in pairs(Props) do
        ...
        if distance < 1 and v.obj ~= nil and DoesEntityExist(v.obj) then
            DeleteEntity(v.obj)
            TriggerServerEvent("vorp_propplacer:DeleteProp" , k)
            Props[k] = nil
            print("deleting...")
...
RegisterNetEvent("vorp_propplacer:GetProps")
AddEventHandler("vorp_propplacer:GetProps", function(data)
    for k,v in pairs(Props) do
        DeleteEntity(v.obj)
    end
    Props = data
...
AddEventHandler('playerSpawned', function()
        TriggerServerEvent('vorp_propplacer:PropRequest')
end)
