...
Citizen.CreateThread(function()
    ...
    if (Vdist(coords.x, coords.y, coords.z, -2.167532, 1542.358, 113.3433) < 1.0) then  --YOUR CHORDS FOR MENU HERE
            DrawTxt("", 0.50, 0.85, 0.7, 0.7, true, 255, 255, 255, 255, true)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('menu', "Nórdicos")
    WarMenu.SetSubTitle('menu', 'Criação')
	
    WarMenu.CreateSubMenu('hgmake', 'menu', 'Remédios')
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('menu') then
           ...
           WarMenu.Display()
            ...
            if WarMenu.MenuButton('Remédios', 'hgmake') then
                end
        ...
        elseif WarMenu.IsMenuOpened('hgmake') then
            ...
            if WarMenu.Button('Óleo BlackShark') then
        ...
        elseif WarMenu.IsMenuOpened('hgmake2') then
            ...
            if WarMenu.Button('Arco e Flechas') then
                
		        TriggerServerEvent("wc_arcofreccebase")
		
            elseif WarMenu.Button('Arco e Flechas Envenenadas') then
            ...
            elseif WarMenu.Button('Hunter') then
            
                TriggerServerEvent("wc_hunter")
                
            elseif WarMenu.Button('Cleaver') then
            
                TriggerServerEvent("wc_ancient")

            elseif WarMenu.Button('Tomahawk Acient') then
            
                TriggerServerEvent("wc_ta")
...
AddEventHandler('indiani:open', function()
    local _source = source
            if weapsmith ~= 0 then
                ...
                WarMenu.OpenMenu('menu')
                TaskStartScenarioInPlace(playerPed, GetHashKey('WORLD_HUMAN_REPAIR_HIGH_WRENCH'), -1, true, false, false, false)
...
AddEventHandler('progressbar:startIndiani', function()
    ...
    local playerPed = PlayerPedId()
    TaskStartScenarioInPlace(playerPed, GetHashKey('WORLD_HUMAN_REPAIR_HIGH_WRENCH'), 27000, true, false, false, false)
