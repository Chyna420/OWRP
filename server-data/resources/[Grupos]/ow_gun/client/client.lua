...
Citizen.CreateThread(function()
    ...
    if (Vdist(coords.x, coords.y, coords.z, -277.1523, 779.1707, 119.604) < 1.0) then  --YOUR CHORDS FOR MENU HERE
            DrawTxt("[~e~G~q~] Para criar.", 0.50, 0.85, 0.7, 0.7, true, 255, 255, 255, 255, true)
            if IsControlJustReleased(0, 0x760A9C6F) then -- g
                --TriggerEvent("indiani:open")
                TriggerServerEvent('gun:checkgroup')
            end
        end
    end
end)

RegisterNetEvent('gun:checkgroupcl')
AddEventHandler('gun:checkgroupcl', function() 
    TriggerEvent("gun:open")
end)
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('menu', "GunStore")
    WarMenu.SetSubTitle('menu', 'Criação')
	
    WarMenu.CreateSubMenu('hgmake', 'menu', 'Manuais')
    WarMenu.CreateSubMenu('hgmake2', 'menu', 'Armas')
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('menu') then
           ...
           WarMenu.Display()
            ...
            if WarMenu.MenuButton('Manuais', 'hgmake') then
                end
        ...
        elseif WarMenu.IsMenuOpened('hgmake') then
            ...
            if WarMenu.Button('DoubleAction BP') then
		
		        TriggerServerEvent("wc_da_bp")

            end
            
            if WarMenu.Button('Schofield BP') then
		
		        TriggerServerEvent("wc_sf_bp")

            end

            if WarMenu.Button('Laço BP') then
		
		        TriggerServerEvent("wc_la_bp")
        ...
        elseif WarMenu.IsMenuOpened('hgmake2') then
            ...
            if WarMenu.Button('DoubleAction Revolver') then
                
		        TriggerServerEvent("wc_darevolver")
		
            elseif WarMenu.Button('Schofield Revolver') then
            
		        TriggerServerEvent("wc_sfrevolver")
        
            --elseif WarMenu.Button('Arco') then
            
                --TriggerServerEvent("wc_arco")
        
            elseif WarMenu.Button('Laço') then
            
		        TriggerServerEvent("wc_laço")
        ...
        elseif WarMenu.IsMenuOpened('hgmake3') then
            ...
            if WarMenu.Button('Aluminio') then
                
		        TriggerServerEvent("wc_al")
		
            elseif WarMenu.Button('Pólvora') then
            
		        TriggerServerEvent("wc_pol")
        
            --elseif WarMenu.Button('Arco') then
            
                --TriggerServerEvent("wc_arco")
        
            elseif WarMenu.Button('Corpo de revolver') then
            
		        TriggerServerEvent("wc_cr")
...
AddEventHandler('gun:open', function()
...
RegisterNetEvent('progressbar:startGun')
AddEventHandler('progressbar:startGun', function()
