local minerblips = {
    {["CaveName"] = "Grizzlies West Mine", x = -1433.591, y = 1182.416, z = 226.3111, ["HasRares"] = false},
    {["CaveName"] = "Elysian Pool Mine", x = 2293.26, y = 1070.23, z = 81.43, ["HasRares"] = true}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        local pos = GetEntityCoords(ped)
        if InArea then
            local AnnesburgCave = {x =-1433.591, y = 1182.416, z = 226.3111}
            ...
            for i=1, #Rocks, 1 do
                ...
                if GetDistanceZTrue(pos,EntCoords) < 3 then
                    ...
                    if nearbyObject then
                        DrawText3D(EntCoords.x, EntCoords.y, EntCoords.z, '[X] Para minerar')
                        if whenKeyJustPressed("X") then
                            ...
                            TriggerServerEvent("VORP_MinerJob:loot", HasRareGems)
                        end
                    end
                end
            end
    ...
    if not IsPedMale(Ped()) then
        ...
        while not HasAnimDictLoaded(dict) do
            ...
            if waiting > 5000 then
                TriggerEvent("vorp:Tip", "Request Animation is broken, Relog", 4, "warning")
                break
            end      
        end
