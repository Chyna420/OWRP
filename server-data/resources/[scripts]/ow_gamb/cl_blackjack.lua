    ...
    while true do
        ...
        while IsPlayerNearCoords(-243.72,771.26,118.22) do
            ...
            if inTable and waiting and not alreadyPressed then
                local text1 = "SPACE para apostar, Espera para passar"
            ...
            elseif (inTable and not alreadyPressed and cardsGiven) then
                DrawScaleText("Esperar a vez", 0.5, 0.9, 0.8)
            end
    ...
    while true do
        ...
        while IsPlayerNearCoords(-243.72,771.26,118.22) do
            ...
            if (inTable and cardsGiven) then 
                DrawScaleText("As tuas cartas:" .. playerCardsText, 0.5, 0.8, 0.8) --player text
                DrawScaleText("Total: " .. playerCount, 0.5, 0.85, 0.8) --player text
                DrawScaleText("Cartas do dealer:" .. dealerCardsText, 0.5, 0.0, 0.8) --dealer text
                DrawScaleText("Total: " .. dealerCount, 0.5, 0.05, 0.8) --dealer text
                DrawScaleText(textWinLosePush, 0.5, 0.5, 1.0) 
            elseif (inTable) then
                DrawScaleText("Espera até a ronda começar" .. playerCardsText, 0.5, 0.9, 0.8) --player text
            end
        end
    end    
end)
...
AddEventHandler('vorp_blackjack:Bet', function(max)
    local x = true
    while x do
        AddTextEntry("FMMC_KEY_TIP8", "Quantas fichas apostas(max:"..max..")")
