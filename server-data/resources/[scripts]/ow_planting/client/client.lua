...
AddEventHandler('poke_planting:fin2', function(object2, x, y, z, key, hash1, hash2, hash3)
    ...
    TriggerEvent("vorp:TipRight", "Sua planta amadureceu", 4000)
...
function harvestPlant(key)
    ...
    TriggerEvent("vorp:TipBottom", "", 10000)
    Wait(10000)
		...
		if myPlants[1] ~= nil then
			...
			for k, v in ipairs(myPlants) do
				if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 7.0 then
					if v.stage == 1 then
						DrawText3D(v.x, v.y, v.z, 'Rega a planta')
					end
					if v.stage == 2 then
						DrawText3D(v.x, v.y, v.z, ' ' .. v.timer)
					end
					if v.stage == 3 then
						DrawText3D(v.x, v.y, v.z, 'Pronto para colher [E]')
					end
					if v.prompt then
...
function animacion()
    ...
    TriggerEvent("vorp:TipBottom", "", 20000)
    Wait(20000)
    ClearPedTasksImmediately(PlayerPedId())
...
function animacion2()
    ...
    TriggerEvent("vorp:TipBottom", "", 7000)
    Wait(7000)
    ClearPedTasksImmediately(PlayerPedId())
end
