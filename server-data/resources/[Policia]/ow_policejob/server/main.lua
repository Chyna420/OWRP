...
AddEventHandler("vanelicenze:cacciatoreditaglie", function(target)
	...
	local count = VorpInv.getItemCount(source, "licenzataglie")
		if count == 0 then
			...
			TriggerClientEvent("vorp:TipRight", _source, "Obteve uma licença de caçador de recompensas", 4000)
		else
			TriggerClientEvent("vorp:TipRight", _source, "Voce ja tem um", 4000)
		end
end)
...
AddEventHandler("vanelicenze:documenti", function(target)
	...
	local count = VorpInv.getItemCount(source, "documento")
		if count < 5 then
			...
			TriggerClientEvent("vorp:TipRight", _source, "Você validou o documento", 4000)
		else
			TriggerClientEvent("vorp:TipRight", _source, "Voce ja tem o suficiente", 4000)
		end
end)
...
AddEventHandler("vanelicenze:cacciatore", function(target)
	...
	local count = VorpInv.getItemCount(source, "licenzacaccia")
		if count == 0 then
			...
			TriggerClientEvent("vorp:TipRight", _source, "Você coletou uma Licença de Caçador", 4000)
		else
			TriggerClientEvent("vorp:TipRight", _source, "Voce ja tem um", 4000)
		end
end)