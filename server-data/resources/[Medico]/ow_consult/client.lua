...
local blips = {
   -- { name = 'Clinica', sprite = -1739686743, x=1369.4, y=-1310.3, z=77.9 },
    { name = 'Clinica', sprite = -1739686743, x=-286.2397, y=804.8081, z=119.4859 },
    ...
    while true do 
        ...
        for _, info in pairs(blips) do 
            ...
            while dist < 1 do
                ...
                DrawTxt("Pressione E para curar a si mesmo ou pressione G para curar o seu parceiro.", 0.5, 0.9, 0.5, 0.5, 255, 255, 255, 255, true)
                ...
                if IsControlJustReleased(0, 0xCEFD9220) then
                    if health >= -1 then
                        ...
                        --notify
                        TriggerEvent("redemrp_notification:start", "Você não pode se curar agora!", 3, "error")
                    end
                elseif IsControlJustReleased(0, 0x760A9C6F) then
                    if not IsPedDeadOrDying(PlayerPedId()) then
                        ...
                        if closestPlayer ~= -1 and closestDistance < 3 and IsPedDeadOrDying(GetPlayerPed(closestPlayer), true) then
                            ...
                            TriggerEvent("redemrp_notification:start", "Sem companheiros para reviver!", 3, "error")
                        end
                    else
                        TriggerEvent("redemrp_notification:start", "Você não pode reviver!", 3, "error")
                    end
                end
            end
        end
