...
AddEventHandler( "vorp_ml_doctorjob:reviveplayer", function(target)
	...
	if medikit > 0 then
		...
		TriggerClientEvent("redemrp_notification:start", _source, "Não tens uma seringa!", 3, "warning")
	end
...
AddEventHandler('doctor:tonicovita', function() 
	...
	local count = VorpInv.getItemCount(_source, "water")
